...
Test of element summary result: #{result.test_status}
      ...
      @log += <<-EOR
Unexpected error in test_status_analyzer method, error: #{e.message}
      EOR
...
Test of attribute, Test message: #{test_attribute.test_message}
      ...
      @log += <<-EOR
Unexpected error in attribute_verification method, error: #{e.message}
      EOR

      save_log
      raise XmlValidatorMethodError, "Unexpected error in attribute_verification method, error: #{e.message}"
    end
...
Value validation, Test message: #{test_of_value.test_message}
      ...
      @log += <<-EOR
Unexpected error in value_validation method, error: #{e.message}
      EOR

      save_log
      raise XmlValidatorMethodError, "Unexpected error in value_validation method, error: #{e.message}"
    end
...
#{"*"*120}
    ...
    rescue XmlValidatorMethodError => ex
      raise XmlValidatorMethodError, ex.message

    rescue Exception => e

      @log += <<-EOR
Unexpected error in child_elements_validation method, error: #{e.message}
      EOR

      save_log
      raise XmlValidatorMethodError, "Unexpected error in child_elements_validation method, error: #{e.message}" unless e.message.include?('method, error:')
    end
...
Test of element summary result: #{ element_test_result.test_status }
      ...
      @log += <<-EOR
Unexpected error in test_of_elements method, error: #{e.message}
      EOR

      save_log
      raise XmlValidatorMethodError, "Unexpected error in test_of_elements method, error: #{e.message}"
    end
...
Unable to locate container element '#{test_spec["container"]}'
      ...
      results = all_nodes.map do |container|
...
#{"="*120}
      ...
      @number_elements = results.size
      @log += <<-EOR

Number of elements tested: #{@number_elements}
      EOR
      ...
      @log += <<-EOR
Unexpected error in run_test method, error: #{e.message}
      EOR

    save_log
    raise  "Unexpected error in run_test method, error: #{e.message}"
    end
  ...
  def report_writer(test_report)
    ...
    File.open("#{@output_folder}/#{filename}", "w") do |f|
      f.puts "Test start time: #{@test_start_time}"
      ...
      f.puts "Test end time: #{@test_end_time}"
      f.puts "Test duration: #{@test_duration}"
      f.puts "Number of tested elements: #{@number_elements}"
    end
  end
end
